...
class PoolHiddenNet(nn.Module):
    ...
    def forward(self, h_states, mask, seq_start_end, end_pos):
        """
        Inputs:
...
class TrajectoryDiscriminator(nn.Module):
    def __init__(
        self, pred_len, n_cond_dim=0, embedding_dim=64, h_dim=64, mlp_dim=1024,
        ...
        input_dim = h_dim + (n_cond_dim if n_cond_dim > 0 else 0)
        real_classifier_dims = [input_dim, mlp_dim, 1]
        mlp_layers = make_mlp(
        ...
        self.real_classifier = nn.Sequential(*mlp_layers, nn.Sigmoid())
        if d_type == 'global':
    ...
    def preprocess_traj(self,traj_abs):

        batch_size, traj_len, _, n_particles = traj_abs.shape

        # → [traj_len, batch_size, n_particles, 2]
        traj_abs = traj_abs.permute(1, 0, 3, 2)
        traj_abs = traj_abs.reshape(traj_len, batch_size * n_particles, 2)

        traj_rel = torch.zeros_like(traj_abs)
        traj_rel[1:] = traj_abs[1:] - traj_abs[:-1]

        # Construct seq_start_end
        seq_start_end = [(i * n_particles, (i + 1) * n_particles) for i in range(batch_size)]
        seq_start_end = torch.LongTensor(seq_start_end).to(traj_abs.device)

        return traj_abs, traj_rel, seq_start_end
    
    def forward(self, traj, label_props_colors, mask):
        """
        Inputs:
